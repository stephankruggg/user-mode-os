...
void Thread::dispatcher()
{
    ...
    if(_main._state == SUSPENDED) {
    	for(int i = 0; i < 5; i++) {
    		db<Thread>(TRC) << " MAIN SUSPENSA. \n";
    	}
    	_main.resume();
    }
...
void Thread::yield()
{
    ...
    {   
    	db<Thread>(TRC) << "Rodando nao esta suspensa nem terminando\n";
        if (_running != &_main)
        {
...
void Thread::resume() 
{
    ...
    if(_state == SUSPENDED) {
        db<Thread>(TRC) << "Removendo da fila de suspensas. \n";
        _state = READY;
        _suspended.remove(_suspended_link);
        db<Thread>(TRC) << "Removido da fila de suspensas. \n";
        if(this != &_main) {
        	int now = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count();
        	_link->rank(now);
        	_ready.insert(_link);
